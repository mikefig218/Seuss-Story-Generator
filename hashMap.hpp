...
class hashMap {
	...
	public:
			hashMap(bool hash1, bool coll1);				 // *****when creating the map, make sure you initialize the values to NULL so you know whether that index has a key in it or not already. The Boolean values initialize the h1 and the c1boolean values, making it easier to control which hash and which collision methods you use.
			...
			int collHash1(int currentindex, string k);			// *********getting index with collision method 1 (note – you may modify the parameters if you don’t need some/need more)
			int collHash2(int currentindex, string k); 			// ******getting index with collision method 2 (note – you may modify the parameters if you don’t need some/need more)
